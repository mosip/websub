...
const string SELECT_AND_UPDATE_FROM_FAILED_DELIVERY_TABLE_BY_SUBID_AND_TIMESTAMP = "WITH c AS (SELECT msg_id FROM websub.message_delivery_failed WHERE subscription_id IN (%s) AND delivery_failed_dtimes>=? ORDER BY delivery_failed_dtimes OFFSET ? LIMIT  ?) UPDATE  websub.message_delivery_failed s SET last_fetch_dtimes=?,upd_by=?,upd_dtimes=? FROM c WHERE s.msg_id = c.msg_id RETURNING s.msg_id";
const string RESTART_REPUBLISH_MESSAGES = "SELECT message,topic FROM websub.message_store WHERE pub_dtimes >=? and pub_dtimes<? and id NOT IN (SELECT  msg_id FROM websub.message_delivery_failed WHERE delivery_failed_dtimes>=? UNION SELECT  msg_id FROM websub.message_delivery_success WHERE delivery_success_dtimes>=?)";
