### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: WEBSUB HUB DB Artifacts deploy
### -- Deploy Module 	: MOSIP WEBSUB
### -- Purpose    		: To deploy MOSIP WEBSUB Database DB Artifacts.       
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('websubuser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating websub database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": websub database user already exist" | tee -a $LOG 2>&1
fi
...
#========================================DB Deployment process completes on WEBSUB DB SERVER======================================
