## MOSIP ID WebSub Database (mosip_websub) Release scripts inventory and deployment/revoke guidelines on postgresql database. 
...
**Example:** For ID Authentication module release script folder is /**mosip_base_directory**>>db_release_scripts>>mosip_websub where all the database release scripts related to Web Sub are available.
...
**Note:** SQL script files name under sql folder are appended with release version numbers( like **1.2.0**_websub-scripts_release.sql), Same version number has to be passed as argument while executing release/revoke shell scripts. Ex. **$bash websub_release_db_deploy.sh websu_release_deploy.properties 1.2.0**
